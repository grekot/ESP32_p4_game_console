...
uint16_t lookup(char key, const PaletteEntry* palette, int n)
{
    ...
    CONSOLE_LOGW(TAG, "brak koloru '%c' w palecie", key);
    return TRANSPARENT;
}
}  // namespace
...
Sprite make_sprite(const char* const* rows, int w, int h, const PaletteEntry* palette, int palette_size)
{
    ...
    if (!px) {
        CONSOLE_LOGE(TAG, "brak pamieci na sprite %dx%d", w, h);
        return s;
    }
    ...
    for (int y = 0; y < h; ++y) {
        ...
        if (len != w && !warned) {
            CONSOLE_LOGW(TAG, "sprite %dx%d: wiersz %d ma %d znakow", w, h, y, len);
            warned = true;
        }
        for (int x = 0; x < w; ++x) {
